...
class Tools:
    ...
    async def compress(self, dl, out, log_msg):
        ...
        cmd = f'''{Var.FFMPEG} -hide_banner -loglevel quiet -progress """{_progress}""" -i """{dl}""" -metadata "Encoded By"="𓆩ÁRîত্র𓆪" -metadata "author"= "𓆩ÁRîত্র𓆪" -preset ultrafast -c:v libx265 -crf {Var.CRF} -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? -metadata:s:s "Encoded By"="𓆩ÁRîত্র𓆪" -metadata:s:a "Encoded By"="𓆩ÁRîত্র𓆪" -metadata:s:v "Encoded By"="𓆩ÁRîত্র𓆪" -threads {self.ffmpeg_threads} """{out}""" -y'''
        process = await asyncio.create_subprocess_shell(
